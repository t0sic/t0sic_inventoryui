$('document').ready(function() {
    ...
    containers.on('drop',function(el,target,source,sibling){
        if (source.id == 'drag1' && source.id != target.id) {
            $.post('http://t0sic_inventoryui/pickup', JSON.stringify({
    ...
    document.getElementById("button").addEventListener("click", function(event) {
        ...
        if (Number(document.getElementById('drop-count').value) > document.getElementsByClassName('selected')[0].innerText) {
            ...
            $('#drag1').append(`<div class="${document.getElementsByClassName('selected')[0].id} default" id="${document.getElementsByClassName('selected')[0].id}">${document.getElementById('drop-count').value}</div>`)
            $(`.${document.getElementsByClassName('selected')[0].id}`).css({
                'padding': '1px',
                'color': 'white',
                'height': '15%',
                'width': '15%',
